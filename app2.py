...
def display_datagrid():
    ...
    new=pd.DataFrame(columns=['batch','branch','courses','sem','attdate','period','regdno','attendance','subjectname'])
    adf={'batch':[],'branch':[],'courses':[],'sem':[],'attdate':[],'period':[],'regdno':[],'attendance':[],'subjectname':[]}
    ...
    for c in range(0,rcount):
        ...
        adf['subjectname'].append(subjectname)
    ...
    st.dataframe(new)
    st.warning("Dear Parent today the following student(s) are absent for The period:(::: Date :"+str(attdate)+":::Period:"+str(period)+":::Subject :"+str(subjectname)+")"+str(absentees))
...
def showconsolidated():
        ...
        tbl=pd.DataFrame(sql_query,columns=['id','batch','branch','courses','sem','attdate','period','regdno','attendance','subjectname'])
        restbl=tbl.pivot(index=['regdno'],columns=['attdate','period'],values=['attendance'])
        ...
        if not restbl.empty:
            ...
            #restbl['sum']=restbl.sum(axis = 1)
            crestbl=restbl.cumsum(axis = 1)
            
            st.dataframe(crestbl)
            #restbl.index = np.arange(1, len(restbl)+1)
            csv = crestbl.to_csv().encode('utf-8')
            st.download_button("Press to Download", csv, name+".csv", "text/csv",  key='download-csv' )



if check_password():
        with st.sidebar:
            choose = option_menu("Swarnandhra Attendance", ["Take Attendance", "Consolidated Report",  "Contact","Logout"],
